...
public class ImageCloud {
	...
	public void GenerateImage(ArrayList<String> list, ArrayList<String> ignoreList) throws Exception {
		...
		// Image file name is assigned to the String
		imageName = Menu.getImageFileName();
		...
		for (i = 0; i < Menu.getMaxWords(); i++) {
			...
			randomX = rand.nextInt(600);// X & Y are random integers (words appear in a random cloud every time)
		...
		graphics.dispose();
		try {
			if (ImageIO.write(image, "PNG", new File(imageName)))
			{
